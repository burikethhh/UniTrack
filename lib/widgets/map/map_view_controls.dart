...
// ─────────────────────────────────────────────────────────────
//  Tracking Mode Dropdown (Staff only)
// ─────────────────────────────────────────────────────────────

enum TrackingMode { gps, manualPin }

/// A compact dropdown that lets staff switch between GPS tracking
/// and manual pin placement mode.
class TrackingModeDropdown extends StatelessWidget {
  final TrackingMode currentMode;
  final ValueChanged<TrackingMode> onModeChanged;

  const TrackingModeDropdown({
    super.key,
    required this.currentMode,
    required this.onModeChanged,
  });
  ...
  static const _modes = [
    (mode: TrackingMode.gps, icon: Icons.gps_fixed, label: 'Auto GPS'),
    (mode: TrackingMode.manualPin, icon: Icons.push_pin, label: 'Manual Pin'),
  ];
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      ...
      borderRadius: BorderRadius.circular(12),
      elevation: 3,
      child: PopupMenuButton<TrackingMode>(
        onSelected: onModeChanged,
        offset: const Offset(0, 44),
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
        itemBuilder: (_) => _modes.map((m) {
          final selected = m.mode == currentMode;
          return PopupMenuItem<TrackingMode>(
            value: m.mode,
            child: Row(
              children: [
                Icon(
        ...
        child: Container(
          ...
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              Icon(current.icon, size: 18, color: AppColors.accent),
              const SizedBox(width: 6),
              Text(
                current.label,
                style: const TextStyle(
...
// ─────────────────────────────────────────────────────────────
@Deprecated('Use MapViewModeDropdown instead')
typedef MapViewControls = MapViewModeDropdown;
